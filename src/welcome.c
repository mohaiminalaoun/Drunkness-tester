...
static void window_load(Window *window) {
  ...
  s_icon_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_BEER);
  GRect bitmap_bounds = gbitmap_get_bounds(s_icon_bitmap);

  s_icon_layer = layer_create(PBL_IF_ROUND_ELSE(
      GRect((bounds.size.w - bitmap_bounds.size.w) / 2, bounds.size.h + WELCOME_WINDOW_MARGIN, bitmap_bounds.size.w, bitmap_bounds.size.h),
      GRect(WELCOME_WINDOW_MARGIN, bounds.size.h + WELCOME_WINDOW_MARGIN, bitmap_bounds.size.w, bitmap_bounds.size.h)
  ));
  layer_set_update_proc(s_icon_layer, icon_update_proc);
  layer_add_child(window_layer, s_icon_layer);

  s_label_layer = text_layer_create(GRect(WELCOME_WINDOW_MARGIN, bounds.size.h + WELCOME_WINDOW_MARGIN + bitmap_bounds.size.h, bounds.size.w - (2 * WELCOME_WINDOW_MARGIN), bounds.size.h));
  text_layer_set_text(s_label_layer, WELCOME_WINDOW_MESSAGE);
...
static void window_appear(Window *window) {
  ...
  const GEdgeInsets icon_insets = {
    .top = WELCOME_WINDOW_MARGIN,
    .left = PBL_IF_ROUND_ELSE((bounds.size.w - bitmap_bounds.size.w) / 2, WELCOME_WINDOW_MARGIN)};
  ...
  const GEdgeInsets finish_insets = {
    .top = WELCOME_WINDOW_MARGIN + bitmap_bounds.size.h + 5 /* small adjustment */,
    .right = WELCOME_WINDOW_MARGIN, .left = WELCOME_WINDOW_MARGIN};
...
static void click_config_provider(void *context) {
  window_single_repeating_click_subscribe(BUTTON_ID_SELECT, REPEAT_INTERVAL_MS, calib_click_handler);

}



void welcome_window_push() {
  if(!window) {
